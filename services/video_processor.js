...
const { analyzeGrammar } = require("./grammar_analysis");
...
async function processVideo(videoUrl, targetLanguage = "zh", userId) {
  try {
    ...
    // 06. 逐行的语法分析
    const subtitlesWithGrammar = await Promise.all(
      translatedSubtitles.map(async (subtitle) => {
        const grammar = await analyzeGrammar(subtitle.originText);
        return { ...subtitle, grammar };
      })
    );
    ...
    const result = {
      ...
      subtitles: subtitlesWithGrammar,
    };
